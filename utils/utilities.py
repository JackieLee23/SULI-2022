...
class UnscaledData:
    def __init__(self, data_loc, X_name, y_name):
...
def see_results(predicted, truth, grid_shape, percentiles, x_vals, tick_labels = None):
...
def baseline_errors(data_loc, y_name):
    test_set = np.load(path.join(data_loc, 'test-set.npz'))
    test_y = test_set[y_name]
